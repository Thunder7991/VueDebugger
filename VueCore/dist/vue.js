...
(function (global, factory) {
  ...
  if (typeof Promise !== "undefined" && isNative(Promise)) {
    //is Promise  micro task
    var p = Promise.resolve();
    timerFunc = function () {
      p.then(flushCallbacks); //微任务
    ...
    !isIE &&
    typeof MutationObserver !== "undefined" && // is MutationObserver micro task
  ...
  /**
   * 方法调用场景是 render 函数是编译生成的。理论上编译生成的 children 都已经是 VNode 类型的，但这里有一个例外，就是 functional component
   * 函数式组件返回的是一个数组而不是一个根节点，所以会通过 Array.prototype.concat 方法把整个 children 数组打平，让它的深度只有一层。
   */
  ...
  /**
   * 方法的调用场景有 2 种，一个场景是 render 函数是用户手写的，当 children 只有一个节点的时候，Vue.js 从接口层面允许用户把 children 写成基础类型用来创建单个简单的文本节点，
   * 这种情况会调用 createTextVNode 创建一个文本节点的 VNode；另一个场景是当编译 slot、v-for 的时候会产生嵌套数组的情况，会调用 normalizeArrayChildren 方法
   */
  function normalizeChildren(children) {
    console.log(43,children);
  ...
  function _createElement(
    ...
    console.log(56,normalizationType);
  ...
  function updateComponentListeners (
    ...
    target = vm;
    console.log(49,listeners,oldListeners);
  ...
  function initMixin (Vue) {
    ...
    Vue.prototype._init = function (options) { //调用init方法() 初始化生命周期，初始化事件中心，初始化渲染，初始化 data、props、computed、watcher 等等
      var vm = this; //vue 实例  Vue{}
    
   console.log('_init' , vm);
      // a uid
      vm._uid = uid$3++;

      var startTag, endTag;
      /* istanbul ignore if */
      if (config.performance && mark) { //性能监测
  ...
  function Vue (options) {
    if (!(this instanceof Vue) //保证了无法直接通过Vue()去调用，只能通过new的方式去创建实例
    ) {
  ...
  initMixin(Vue); //定义原型上的init方法（内部方法）
  ...
  Vue.prototype.$mount = function (
    ...
    /* istanbul ignore if */
    if (el === document.body || el === document.documentElement) {   //Vue 不能挂载在 body、html 这样的根节点上
      warn(
